...
class Home extends CI_Controller {
	...
	public function profile_add(){
		$this->authenticate->is_user_login();
		$data['mobile_no'] =  $this->session->userdata('mobnumber_ki');
	...
	public function profileEdit(){
		$this->authenticate->is_user_login();
		$id=$this->session->userdata('userId_ki');
	...
	public function rechargenumber(){
		...
		elseif ($result[0]['BASIC_STATE'] != 'F0B') {
			...
			$this->session->set_flashdata('mobilenumber_ki', $number);
			$this->session->set_flashdata('amount_ki', $amount);
	...
	public function activateOffer($id){
	 ...
	 $username =  $this->session->userdata('username_ki');
	...
	public function test(){
		...
		print_r($this->api->QueryOfferInfo('63002096'));
	}
}
