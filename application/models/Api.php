...
Class Api extends CI_Model
{
   ...
   function basic_detail($number){
      $url = apiHost.'/nauru/getUserBasicInfo';
   ...
   function offerList(){
      $url = apiHost.'/nauru/getOfferList';
      // use key 'http' even if you send the request to https://...
      $options = array(
          'http' => array(
              'header'  => "Content-type: application/x-www-form-urlencoded\r\n",
...
function voice_cdr($number){
      $url = apiHost.'/nauru/getVoice_CDR';
   ...
   function data_cdr($number){
    ...
    function rechargCDR($number){
      $ch = curl_init();
      curl_setopt($ch, CURLOPT_URL,apiHost.'/nauru/getRechargeCDR');
    ...
    function transferCDR($number){
      $ch = curl_init();
      curl_setopt($ch, CURLOPT_URL,apiHost.'/nauru/getTransferCDR');
      curl_setopt($ch, CURLOPT_POST, 1);
      curl_setopt($ch, CURLOPT_POSTFIELDS,http_build_query(array('userId' => $number)));
